 ...
 *
 * [이미 시트를 쓰고 있다면] 2027 연출 관련 학과 정보(청강문화산업대 연출·극작전공,
 * 실제 확인된 원서접수/면접 일정, 실기·포트폴리오 준비 할일)를 추가하려면:
 *  1) 이 파일 전체를 기존 Apps Script 코드에 덮어쓰기 저장
 *  2) 함수 선택 드롭다운에서 add2027DirectingUpdate 선택 후 ▶ 실행 (기존 데이터는 보존, id 기준으로 추가/정정만 됨. 여러 번 실행해도 안전)
...
function seedInitialData() {
  ...
  // 6개 대학 시드 + 연출 전공 특화 후보 1개(청강문화산업대)
  ...
  if (u.getLastRow() <= 1) {
    const seed = [
      ...
      ['u3', 'jeokjeong', '강남대',     '미디어커뮤니케이션학과', '서류면접전형(학종)',           '없음', '서류70+면접30', '경기 용인 기흥 (분당선 기흥역 셔틀). 남양주→1시간 30분. 학종 집중 추천. 2027 면접평가 11.7~8(예정)', '준비중', '4년제'],
      ...
      ['u7', 'jeokjeong', '청강문화산업대', '공연예술스쿨 연출·극작전공', '수시1차 면접전형(서류+포트폴리오면접)', '없음', '면접100%(포트폴리오, 1인 15분내외)', '경기 이천시 (남양주→약1시간40분~2시간, 셔틀 확인 필요). 학과명에 "연출"이 명시된 유일한 후보 — 동아리 부기장·조명·연출 경험 어필에 최적. 1차 서류경쟁 치열. 수시1차 접수 26.9.7~9.30·면접 10.20~21 / 수시2차 접수 11.11~25·면접 12.5~6', '신규검토', '전문대']
    ];
  ...
  if (q.getLastRow() <= 1) {
    const qa = [
      ...
      ['q16', '(청강문화산업대 연출·극작전공) 왜 배우가 아니라 연출을 하고 싶나요?', '무대 위에서 가장 오래 남아 있었던 기억은 제 대사가 아니라 조명 큐를 넘기던 순간이었습니다. 배우 한 명의 감정보다 무대 전체의 흐름을 설계하고 완성하는 역할에 더 큰 성취감을 느껴 연출을 선택했습니다.', '진로'],
      ['q17', '(청강문화산업대) 포트폴리오에서 가장 자신 있는 작업은 무엇인가요?', '2학년 축제 연극에서 부기장으로 시나리오 각색부터 조명 큐시트, 무대 동선까지 직접 설계한 기획서입니다. 텍스트 각색안, 조명 큐시트, 리허설 사진을 묶어 포트폴리오로 정리했고, 문제 상황을 어떻게 조율해 완성했는지 함께 설명할 수 있습니다.', '학과지식']
    ];
  ...
  if (t.getLastRow() <= 1) {
    ...
    const todos = [
      ...
      // ↓ 연출 관련 학과 실기·포트폴리오 준비 (2026.08 추가)
      ['t24','부모','정보수집','서일대 입학처(02-490-7201) 전화 — 영화방송공연예술학과 실기고사 종목·평가기준 확인','2026-09-05','high',false,now],
      ['t25','부모','정보수집','백석예술대 입학처(041-550-0800) 전화 — 공연예술경영과 실기 세부내용 확인','2026-09-05','high',false,now],
      ['t26','부모','정보수집','청강문화산업대 연출·극작전공 지원 여부 최종 결정 (수시1차 접수 9.30 마감 전)','2026-09-20','high',false,now],
      ['t27','수빈','실기준비','연출·기획 포트폴리오 제작 — 동아리(루미너스) 기획서·큐시트·조명디자인·무대사진·시나리오 정리','2026-09-10','high',false,now],
      ['t28','수빈','실기준비','청강 면접(15분·포트폴리오 100%) 대비 — 연출 의도·역할 설명 스피치 원고 작성 및 반복 연습','2026-09-25','high',false,now],
      ['t29','수빈','실기준비','서일대·백석예술대 실기 종목 확정 후 SAN학원과 자유연기/즉흥상황극 실기 커리큘럼 시작','2026-09-20','high',false,now],
      ['t30','수빈','실기준비','즉흥 상황극 구성 연습 (공연예술 실기 공통 대비 — 상황 카드 뽑고 1분 내 장면 구성)','2026-10-01','mid',false,now],
      ['t31','수빈','실기준비','자기소개 스피치(1~2분) 완성 및 거울·영상 반복 연습 — 면접·실기 공통 사용','2026-08-20','high',false,now]
    ];
  ...
  if (s.getLastRow() <= 1) {
    const sched = [
      ...
      ['s2','2026-09-07','수시 원서 접수 시작 (공통기간)','원서','대교협 공통규정 9.7~9.11, 대학별 3일 이상 자율 지정 — 안양대·한신대·강남대·동서울대 정확 접수일은 학교별 모집요강 재확인'],
      ['s3','2026-09-11','수시 원서 접수 마감 (공통기간)','원서','이 기간 내 대학별 실제 접수일 상이 — 어디가/입학처 공지로 최종 확인'],
      ...
      ['s9','2026-12-27','정시 원서 접수 시작 (예상)','원서','대진대·호원대·청운대 등 확인'],
      ['s10','2027-01-06','정시 원서 접수 마감 (예상)','원서','대진대·호원대·청운대 등 확인'],
      // ↓ 2027학년도 실제 모집요강 기준 추가 (2026.08 확인, 연출·공연예술 계열)
      ['s13','2026-09-07','청강문화산업대(연출·극작전공) 수시1차 원서접수 시작','원서','공연예술스쿨 연출·극작전공 — "연출" 명시 신규 후보'],
      ['s14','2026-09-30','청강문화산업대 수시1차 원서접수 마감','원서',''],
      ['s15','2026-10-20','청강문화산업대 수시1차 면접(포트폴리오)','면접','1인 15분 내외, 면접100%. 활동 포트폴리오 지참'],
      ['s16','2026-11-07','강남대 미디어커뮤니케이션 학종 면접평가','면접','2027학년도 확정 일정(주말 대면면접), 리더십·기획력 어필'],
      ['s17','2026-11-11','청강문화산업대 수시2차 원서접수 시작','원서','1차 미합격/미등록 시 대비'],
      ['s18','2026-11-25','청강문화산업대 수시2차 원서접수 마감','원서',''],
      ['s19','2026-12-05','청강문화산업대 수시2차 면접(포트폴리오)','면접','1인 15분 내외, 면접100%'],
      ['s20','2026-09-15','서일대·백석예술대 실기고사 종목·일정 입학처 확인 마감','실기','서일대 02-490-7201 / 백석예술대 041-550-0800 — 실기 세부내용 미공개분 직접 확인 후 학원 실기 커리큘럼 확정']
    ];
...
/* ============================================================
 * 2027 연출 관련 학과 업데이트 (이미 시트가 채워진 상태에서 실행)
 *  - 실행 방법: Apps Script 편집기 상단 함수 선택 드롭다운에서
 *    "add2027DirectingUpdate" 선택 후 ▶ 실행 (여러 번 실행해도 안전 - id 기준 upsert)
 * ============================================================ */
function add2027DirectingUpdate() {
  const now = new Date().toISOString();

  // 일정: 공통 원서접수 기간 정정 + 청강문화산업대(연출·극작전공)·강남대·실기 확인 일정 추가
  const scheduleRows = [
    ['s2','2026-09-07','수시 원서 접수 시작 (공통기간)','원서','대교협 공통규정 9.7~9.11, 대학별 3일 이상 자율 지정 — 안양대·한신대·강남대·동서울대 정확 접수일은 학교별 모집요강 재확인'],
    ['s3','2026-09-11','수시 원서 접수 마감 (공통기간)','원서','이 기간 내 대학별 실제 접수일 상이 — 어디가/입학처 공지로 최종 확인'],
    ['s13','2026-09-07','청강문화산업대(연출·극작전공) 수시1차 원서접수 시작','원서','공연예술스쿨 연출·극작전공 — "연출" 명시 신규 후보'],
  ...
  scheduleRows.forEach(r => _upsert('schedule', _rowObj('schedule', r)));

  // 대학: 연출 전공명이 명시된 신규 후보 (청강문화산업대)
  const univRows = [
    ['u7', 'jeokjeong', '청강문화산업대', '공연예술스쿨 연출·극작전공', '수시1차 면접전형(서류+포트폴리오면접)', '없음', '면접100%(포트폴리오, 1인 15분내외)', '경기 이천시 (남양주→약1시간40분~2시간, 셔틀 확인 필요). 학과명에 "연출"이 명시된 유일한 후보 — 동아리 부기장·조명·연출 경험 어필에 최적. 1차 서류경쟁 치열. 수시1차 접수 26.9.7~9.30·면접 10.20~21 / 수시2차 접수 11.11~25·면접 12.5~6', '신규검토', '전문대']
  ];
  univRows.forEach(r => _upsert('universities', _rowObj('universities', r)));

  // 할 일: 실기고사 확인 + 포트폴리오·실기 준비
  const todoRows = [
    ['t24','부모','정보수집','서일대 입학처(02-490-7201) 전화 — 영화방송공연예술학과 실기고사 종목·평가기준 확인','2026-09-05','high',false,now],
    ['t25','부모','정보수집','백석예술대 입학처(041-550-0800) 전화 — 공연예술경영과 실기 세부내용 확인','2026-09-05','high',false,now],
    ['t26','부모','정보수집','청강문화산업대 연출·극작전공 지원 여부 최종 결정 (수시1차 접수 9.30 마감 전)','2026-09-20','high',false,now],
  ...
  Logger.log('2027 연출 관련 학과 업데이트 완료: schedule +10(2 정정), universities +1, todos +8, interview +2');
}
